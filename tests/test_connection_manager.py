...
class ConnectionManagerTestCase(TestCase):
    ...
    def test_get_or_create(self):
        ...
        conn, is_created = Connection.objects.get_or_create(created_user=self.user,
                                                            with_user=user_2)
        self.assertTrue(is_created)

        conn_3, is_created = Connection.objects.get_or_create(created_user=self.user,
                                                              with_user=user_2)
    ...
    def test_get_for_users(self):
        ...
        conn_db = Connection.objects.get_for_users(user_1=self.user,
                                                   user_2=user_2)
        self.assertEqual(conn, conn_db)

        conn_db_2 = Connection.objects.get_for_users(user_1=user_2,
                                                     user_2=self.user)
